...
% Without constraints
constraints.lbx(1:state_length*(N+1)+input_length*N,1) = -inf;
constraints.ubx(1:state_length*(N+1)+input_length*N,1) = inf;
...
while(loop_num < T / Ts)
    ...
    for k = 1:N
        ...
        constraints.p((state_length+input_length)*k +4:(state_length+input_length)*k +6) = [0, 0, 0]; % u_cl(loop_num+1,:); % 
    end
